...
foreach($event_array as $event) { 
	...
	<div style="font-size: 1.8rem;line-height: 1.5">
			...
			<?php endif; ?>
			<h3><a href="<?php echo get_bloginfo('url'); ?>/events/events-calendar/?mc_oid=<?php echo $thisevent->event_id; ?>" style="text-decoration:none;font-size:1.2em;text-transform:none;"><?php echo stripslashes($thisevent->event_title); ?></a></h3>
        <?php 
...
/*  Apologies to future programmers.
    This addresses a problem where the last event wouldn't print in the loop, so had to be printed outside of the loop. Ideally this would be in a function as to not repeat code... but the bug is being fixed with only hours remaining before I have to stop working on this for good.
*/
...
<div style="font-size: 1.8rem;line-height: 1.5">
			...
			<?php else: ?>
				<!-- <img src="//westseattle.soundgenerations.org/wp-content/uploads/sites/6/2016/07/header-home-west-seattle-seniors.jpg" style="float:right;width:200px;" /> -->
			<?php endif; ?>
			<h3><a href="<?php echo get_bloginfo('url'); ?>/events/events-calendar/?mc_id=<?php echo $thisevent->occur_id; ?>" style="text-decoration:none;font-size:1.2em;text-transform:none;"><?php echo stripslashes($thisevent->event_title); ?></a></h3>
        <?php 
        foreach($thiseventdays as $thisdaykey => $thisdayvalue) {
            $thiseventdates .= '<span style="font-weight:bolder;">' . $thisdaykey . ', ';
            foreach($thisdayvalue as $thistimekey => $thistimevalue) {
